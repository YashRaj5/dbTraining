...
-- MAGIC %python
-- MAGIC print(spark.table("pre_delete_vw").selectExpr("int(sum(grams))").first()[0])
...
RESTORE TABLE beans VERSION AS OF 4;
...
-- MAGIC assert spark.table("beans").count() == 5, "Make sure you reverted to the verison after deleting records but before merging"
...
-- MAGIC %python
-- MAGIC dbutils.fs.ls('dbfs:/mnt/dbacademy-users/yash.raj@hanu.com/data-engineer-learning-path/database.db/beans')
